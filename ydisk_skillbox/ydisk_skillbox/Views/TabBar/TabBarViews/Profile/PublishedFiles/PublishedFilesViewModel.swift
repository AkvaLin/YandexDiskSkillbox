...
struct PublishedFilesViewModel {
    var publishedFilesModel: Observable<[TableViewCellModel]> = Observable([])
    ...
    private func fetch(completionHandler: @escaping (Int, [TableViewCellModel]?) -> Void) {
        ...
        guard let url = URL(string: "https://cloud-api.yandex.net/v1/disk/resources/public?offset=\(count.value!)") else {
        ...
        let task = URLSession.shared.dataTask(with: request, completionHandler: { data, response, error in
            ...
            do {
                ...
                var publishedFiles = [TableViewCellModel]()
                ...
                isFetchInProgress.value = false
                completionHandler(resp.statusCode, publishedFiles)
            } catch let error {
                print(error)
                isFetchInProgress.value = false
    ...
    func fetchData(completionHandler: @escaping (Int) -> Void) {
        ...
        fetch(completionHandler: { handler, data in
            ...
            if let data = data {
                publishedFilesModel.value?.append(contentsOf: data)
            }
        })
    }
    ...
    func updateData(completionHandler: @escaping (Int) -> Void) {
        ...
        fetch(completionHandler: { handler, data in
            completionHandler(handler)
            if let data = data {
                publishedFilesModel.value = data
            }
        })
    }
    
    func removePublication(path: String, completionHandler: @escaping (Int) -> Void) {
        
        guard let replaced = path.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed) else { return }
        
        guard let finalPath = URL(string: replaced) else { return }
        
        guard let url = URL(
            string: "https://cloud-api.yandex.net/v1/disk/resources/unpublish?path=\(finalPath)"
        ) else { return }
...
extension PublishedFilesViewModel {
    ...
    private func getAllFiles() {
        persistentContainer.performBackgroundTask() { (context) in
            ...
            folderFetchRequest.predicate = NSPredicate(format: "path == 'Published'")
            ...
            publishedFilesModel.value = returnFiles
        }
    }
    ...
    func deleteFile(path: String) {
        persistentContainer.performBackgroundTask() { (context) in
            ...
            let folder = try! context.fetch(folderFetchRequest)
            if folder[0].items!.contains(item[0]) {
                item[0].removeFromFolder(folder[0])
            }
