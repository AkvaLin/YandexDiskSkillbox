...
class FilesViewController: UIViewController {
    ...
    private let noFilesLbl: UILabel = {
        let lbl = UILabel()
        lbl.text = "У вас пока нет файлов"
    ...
    private let viewModel = FilesViewModel(container: (UIApplication.shared.delegate as!
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if viewModel.filesModel.value?[indexPath.row].type == "dir" {
            ...
            vc.onRename = { [weak self] (path, name) in
                self?.viewModel.filesModel.value?[indexPath.item].name = name
...
extension FilesViewController: UITableViewDataSourcePrefetching {
