//
//  RecentlyFilesViewController.swift
//  ydisk_skillbox
//
...
class RecentlyFilesViewController: UIViewController {
    ...
    private let viewModel = RecentlyFilesViewModel(container: (UIApplication.shared.delegate as!
    ...
    override func viewDidLoad() {
        ...
        view.backgroundColor = .systemBackground
        navigationController?.navigationBar.topItem?.title = "Последние".localized
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        viewModel.recenltyFilesModel.bind { [weak self] _ in
            DispatchQueue.main.async {
                self?.tableView.reloadData()
                self?.hud.dismiss(animated: true)
                self?.refreshControl.endRefreshing()
                if let value = self?.viewModel.recenltyFilesModel.value {
                    if value.isEmpty {
    ...
    private func fetchData() {
        DispatchQueue.global(qos: .userInteractive).async {
            self.viewModel.fetch(completionHandler: { [weak self] statusCode in
                if statusCode == 401 {
                    ClearData.deleteAll()
                    DispatchQueue.main.async {
...
extension RecentlyFilesViewController: UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return viewModel.recenltyFilesModel.value?.count ?? 0
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var image = UIImage()
        let name = viewModel.recenltyFilesModel.value?[indexPath.row].name ?? ""
        let size = viewModel.recenltyFilesModel.value?[indexPath.row].size != nil ? Units(bytes: Int64((viewModel.recenltyFilesModel.value?[indexPath.row].size)!)).getReadableUnit() : ""
        let date = viewModel.recenltyFilesModel.value?[indexPath.row].date ?? ""
        let time = viewModel.recenltyFilesModel.value?[indexPath.row].time ?? ""
        
        if let data = viewModel.recenltyFilesModel.value?[indexPath.row].preview {
            image = UIImage(data: data) ?? UIImage(systemName: "questionmark.app")!
        } else {
            if viewModel.recenltyFilesModel.value?[indexPath.row].type == "dir" {
                image = UIImage(systemName: "folder.fill")!
            } else if viewModel.recenltyFilesModel.value?[indexPath.row].type == "file" {
...
extension RecentlyFilesViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        tableView.deselectRow(at: indexPath, animated: true)
        if viewModel.recenltyFilesModel.value?[indexPath.row].type == "dir" {
            let vc = DirViewController(name: viewModel.recenltyFilesModel.value?[indexPath.row].name ?? "",
                                       path: viewModel.recenltyFilesModel.value?[indexPath.row].path ?? ""
            )
            self.navigationController?.pushViewController(vc, animated: true)
        } else {
            let vc = FileViewController(name: viewModel.recenltyFilesModel.value?[indexPath.row].name ?? "",
                                        date: viewModel.recenltyFilesModel.value?[indexPath.row].date ?? "",
                                        time: viewModel.recenltyFilesModel.value?[indexPath.row].time ?? "",
                                        path: viewModel.recenltyFilesModel.value?[indexPath.row].path ?? "",
                                        mediaType: viewModel.recenltyFilesModel.value?[indexPath.row].mediaType ?? "",
                                        mimeType: viewModel.recenltyFilesModel.value?[indexPath.row].mimeType ?? ""
            )
            vc.onDelete = { [weak self] path in
                self?.viewModel.recenltyFilesModel.value?.remove(at: indexPath.item)
                self?.viewModel.deleteFile(path: path)
                tableView.deleteRows(at: [indexPath], with: .none)
            }
            vc.onRename = { [weak self] (path, name) in
                self?.viewModel.recenltyFilesModel.value?[indexPath.item].name = name
