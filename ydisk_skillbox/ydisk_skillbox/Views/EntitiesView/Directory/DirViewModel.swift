...
struct DirViewModel {
    ...
    init(container: NSPersistentContainer, folderPath: String) {
    ...
    private func fetch(path: String,
                       completionHandler: @escaping (Int, [TableViewCellModel]?) -> Void) {
        createFolder()
        
        guard let replaced = path.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed) else { return }
        
        guard let url = URL(string: "https://cloud-api.yandex.net/v1/disk/resources?path=\(replaced)&offset=\(count.value!)") else {
    ...
    func fetchData(path: String, completionHandler: @escaping (Int) -> Void) {
        ...
        fetch(path: path, completionHandler: { handler, data in
    ...
    func updateData(path: String, completionHandler: @escaping (Int) -> Void) {
...
extension DirViewModel {
    ...
    private func getAllFiles() {
        persistentContainer.performBackgroundTask() { (context) in
            ...
            folderFetchRequest.predicate = NSPredicate(format: "path == %@", folderPath)
    ...
    private func createFolder() {
        persistentContainer.performBackgroundTask() { (context) in
            do {
                ...
                let pred = NSPredicate(format: "path == %@", folderPath)
